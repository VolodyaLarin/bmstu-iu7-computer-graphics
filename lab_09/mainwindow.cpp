    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->canvas = canvas;
    ui->figureTable->setItemDelegate(new MyDelegate);
...
void MainWindow::on_addClipperPoint_clicked()
{
    fmanager.addClipperPoint(QPoint(0,0));
}

void MainWindow::on_figureTable_itemChanged(QTableWidgetItem *item)
{
    canvas->drawFigure();
}

void MainWindow::on_addFigurePoint_clicked()
{
    fmanager.addFigurePoint(QPoint(0,0));
}

void MainWindow::on_deleteFigurePoint_clicked()
{
    int rows = ui->figureTable->rowCount();
    if (!rows) return;
    ui->figureTable->setRowCount(rows - 1);

    canvas->drawFigure();
}

void MainWindow::on_deleteClipperPoint_clicked()
{
...
{
    static auto old_name = ui->drawFigure->text();
    if (!canvas->is_draw_clipper && !canvas->is_adding_line)
    {
        ui->figureTable->setRowCount(0);
        canvas->is_draw_figure = true;
        canvas->drawFigure();
        ui->drawFigure->setText("Замкнуть");
    }
    else if (canvas->is_draw_figure) {
        canvas->is_draw_figure = false;
        canvas->is_adding_line = false;
        canvas->drawFigure();
        ui->drawFigure->setText(old_name);
    }
...
void MainWindow::on_drawClipper_clicked()
{
    ...
    if (!canvas->is_draw_figure && !canvas->is_adding_line)
    {
        ui->clipperTable->setRowCount(0);
